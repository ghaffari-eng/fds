...
!      NWP                  = SF%N_CELLS_INI
   ...
   IF (ICYC>WALL_INCREMENT) THEN
      IF (ONE_D%INTERNAL_RADIATION) THEN
         Q_NET_F = Q_CON_F + B1%Q_EMBER
         Q_NET_B = Q_CON_B
      ELSE
         Q_NET_F = B1%Q_RAD_IN - B1%EMISSIVITY  *SIGMA*B1%TMP_F**4 + Q_CON_F + B1%Q_EMBER
   ...
   ELSEIF (.NOT.RADIATION .OR. ONE_D%INTERNAL_RADIATION .OR. ISOLATED_THIN_WALL) THEN
      ...
      QDXKF  = (HTCF*B1%TMP_G  + Q_LIQUID_F + B1%Q_EMBER)/(KODXF+RFACF)
   ELSE
      RFACF  = 0.5_EB*HTCF + 2._EB*B1%EMISSIVITY*SIGMA*B1%TMP_F**3
      RFACF2 = (KODXF-RFACF)/(KODXF+RFACF)
      QDXKF  = (HTCF*B1%TMP_G  + Q_LIQUID_F + B1%Q_RAD_IN + 3._EB*B1%EMISSIVITY*SIGMA*B1%TMP_F**4 + B1%Q_EMBER) / &
               (KODXF+RFACF)
   ENDIF
