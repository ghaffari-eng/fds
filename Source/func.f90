...
TYPE(BOUNDARY_PROP1_TYPE), POINTER :: B1_LP
REAL(EB) :: F_N,POWER,X,LOGIT,WIND_SPEED,EMBER_MASS_G,EMBER_TMP_C

M  => MESHES(NM)
LP => M%LAGRANGIAN_PARTICLE(IP)
BC => M%BOUNDARY_COORD(LP%BC_INDEX)
SF => SURFACE(M%LS_SURF_INDEX(BC%IIG,BC%JJG))
B1_LP => M%BOUNDARY_PROP1(LP%B1_INDEX)

IGNITION_TYPE_SELECT: SELECT CASE (SF%EMBER_IGNITION_TYPE)

CASE (0) IGNITION_TYPE_SELECT

   ! Original power-based normal CDF model

   IF (SF%EMBER_IGNITION_POWER_MEAN<0._EB) RETURN

   ! Compute the energy generation rate of the ember

   POWER = (B1_LP%Q_RAD_OUT - B1_LP%Q_RAD_IN - B1_LP%Q_CON_F)* &
            0.001_EB*B1_LP%AREA

   ! Compute the CDF of the POWER
   F_N = 0.5_EB*(1._EB + ERF((POWER-SF%EMBER_IGNITION_POWER_MEAN)/ &
         (SR2*SF%EMBER_IGNITION_POWER_SIGMA)))
...
CASE (2) IGNITION_TYPE_SELECT

   ! Heat-transfer based ignition: handled by solid phase solver via Q_EMBER
   RETURN

END SELECT IGNITION_TYPE_SELECT
