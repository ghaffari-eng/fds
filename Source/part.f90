...
INTEGER :: IP,IC_NEW,IIG_OLD,JJG_OLD,KKG_OLD,IIG_TRY,JJG_TRY,KKG_TRY,IW,IW_RESET,IC_OLD,IOR_HIT,&
           N_ITER,ITER,I_COORD,J_COORD,IC_TRY,IOR_ORIGINAL,ICC
TYPE (LAGRANGIAN_PARTICLE_TYPE), POINTER :: LP
TYPE (LAGRANGIAN_PARTICLE_CLASS_TYPE), POINTER :: LPC
TYPE (SURFACE_TYPE), POINTER :: SF,SF_W
TYPE(CFACE_TYPE), POINTER :: CFA_OLD,CFA_NEW
TYPE(BOUNDARY_PROP1_TYPE), POINTER :: B1,B1_E,B1_W
TYPE(BOUNDARY_COORD_TYPE), POINTER :: BC,CFA_OLD_BC,CFA_NEW_BC
REAL(EB), PARAMETER :: ONTHHALF=0.5_EB**ONTH, B_1=1.7321_EB
REAL(EB) :: TMP_EMBER,TMP_SURFACE,A_EMBER_PROJ,A_CELL_W,Q_EMBER_RAD,Q_EMBER_CON
...
! Reset ember heat flux accumulation for all wall cells

DO IW_RESET=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   BOUNDARY_PROP1(WALL(IW_RESET)%B1_INDEX)%Q_EMBER = 0._EB
   BOUNDARY_PROP1(WALL(IW_RESET)%B1_INDEX)%EMBER_COUNT = 0
ENDDO
...
PARTICLE_LOOP: DO IP=1,NLP
   ...
   ! Deposit ember heat flux onto the underlying solid surface

   IF (LP%EMBER .AND. LP%WALL_INDEX>0) THEN
      SF_W => SURFACE(WALL(LP%WALL_INDEX)%SURF_INDEX)
      IF (SF_W%EMBER_HEAT_TRANSFER) THEN
         B1_E => BOUNDARY_PROP1(LP%B1_INDEX)
         B1_W => BOUNDARY_PROP1(WALL(LP%WALL_INDEX)%B1_INDEX)
         TMP_EMBER   = B1_E%TMP_F
         TMP_SURFACE = B1_W%TMP_F
         A_EMBER_PROJ = PI*LP%RADIUS**2
         A_CELL_W = B1_W%AREA
         IF (A_CELL_W < TWO_EPSILON_EB) A_CELL_W = 1._EB
         Q_EMBER_RAD = SF_W%EMBER_EMISSIVITY*SIGMA* &
                       (TMP_EMBER**4 - TMP_SURFACE**4)
         Q_EMBER_CON = SF_W%EMBER_CONTACT_HTC* &
                       (TMP_EMBER - TMP_SURFACE)
         B1_W%Q_EMBER = B1_W%Q_EMBER + &
            SF_W%EMBER_CONTACT_FRACTION* &
            (A_EMBER_PROJ/A_CELL_W)* &
            (Q_EMBER_RAD + Q_EMBER_CON)*LP%PWT
         B1_W%EMBER_MASS_ACCUMULATED = &
            B1_W%EMBER_MASS_ACCUMULATED + LP%MASS*LP%PWT
         B1_W%EMBER_COUNT = B1_W%EMBER_COUNT + 1
      ENDIF
   ENDIF
