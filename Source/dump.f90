...
! Set ODE solver
   ...
   REACTION_LOOP: DO NR=1,N_REACTIONS
      ...
      IF (SUPPRESSION .AND. RN%FAST_CHEMISTRY .AND. RN%PRIORITY==1) THEN
         ...
         IF (EXTINCT_MOD==EXTINCTION_3) THEN
            WRITE(LU_OUTPUT,'(6X,A,F8.1)') 'Critical Extinction Strain Rate (1/s):  ', RN%CRITICAL_EXTINCTION_STRAIN_RATE
            WRITE(LU_OUTPUT,'(6X,A,F8.3)') 'Critical Damkohler Number:              ', DA_EXTINCTION
         ENDIF
      ENDIF
      IF (SIM_MODE/=DNS_MODE) THEN
...
IND_SELECT: SELECT CASE(IND)
   ...
   CASE(195) ! DAMKOHLER NUMBER
      DA_TMP = REACTION(1)%CRITICAL_EXTINCTION_STRAIN_RATE * MIX_TIME(II,JJ,KK)
      GAS_PHASE_OUTPUT_RES = DA_TMP

   CASE(196) ! PASR REACTING FRACTION
      DA_TMP = REACTION(1)%CRITICAL_EXTINCTION_STRAIN_RATE * MIX_TIME(II,JJ,KK)
      GAS_PHASE_OUTPUT_RES = DA_TMP / (1._EB + DA_TMP)
